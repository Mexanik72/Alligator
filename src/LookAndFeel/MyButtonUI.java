...
public class MyButtonUI extends BasicToggleButtonUI {
	...
	public static final Stroke focusStroke = new BasicStroke(1,
			BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND, 5, new float[] { 3,
					3 }, 0);
	...
	private static Integer integer = null;
	...
	public MyButtonUI(final AbstractButton button,
			final boolean changeForeground) {
	...
	public MyButtonUI(final AbstractButton button,
			final boolean changeForeground, float mouseoverTransparency) {
		super();
	...
	public void setColor(Color color, int alpha) {
		this.topBg = new Color(color.getRed(), color.getGreen(),
				color.getBlue(), alpha);
		this.bg = color;
	}
	...
	public void setStaticColor(Color topBg, Color bottomBg) {
		...
		this.disabledStaticBottomBg = new Color(Math.min(255,
				bottomBg.getRed() + 5), Math.min(255, bottomBg.getGreen() + 5),
				Math.min(255, bottomBg.getBlue() + 5));
	}

	public void setStaticBorderColor(Color staticBorderColor,
			Color staticDisabledBorderColor) {
	...
	public java.util.List<Integer> getRoundedSides() {
		...
		if (!isSharpTopLeft() && !isSharpBottomRight() && !isSharpBottomLeft()
				&& !isSharpBottomRight()) {
			rounded.add(-1);
		} else if (isSharpTopLeft() && isSharpBottomRight()
				&& isSharpBottomLeft() && isSharpBottomRight()) {
			rounded.add(0);
		} else {
			if (!isSharpTopLeft()) {
				rounded.add(1);
	...
	public void paint(Graphics g, JComponent c) {
		...
		if (!sharpTopRight) {
			...
			gp1.quadTo(c.getWidth() - 2, 1, c.getWidth() - 2,
					1 + midRounding / 2);
			gp2.lineTo(c.getWidth() - midRounding / 2 - 3, 2);
			gp2.quadTo(c.getWidth() - 3, 2, c.getWidth() - 3,
					2 + midRounding / 2);
		} else {
		...
		if (!sharpBottomRight) {
			...
			gp0.quadTo(c.getWidth() - 1, c.getHeight() - 1, c.getWidth()
					- maxRounding / 2 - 1, c.getHeight() - 1);
			gp1.lineTo(c.getWidth() - 2, c.getHeight() - midRounding / 2 - 2);
			gp1.quadTo(c.getWidth() - 2, c.getHeight() - 2, c.getWidth()
					- midRounding / 2 - 2, c.getHeight() - 2);
			gp2.lineTo(c.getWidth() - 3, c.getHeight() - midRounding / 2 - 3);
			gp2.quadTo(c.getWidth() - 3, c.getHeight() - 3, c.getWidth()
					- midRounding / 2 - 3, c.getHeight() - 3);
		} else {
		...
		if (!sharpBottomLeft) {
			...
			gp0.quadTo(0, c.getHeight() - 1, 0, c.getHeight() - maxRounding / 2
					- 1);
			gp1.lineTo(1 + midRounding / 2, c.getHeight() - 2);
			gp1.quadTo(1, c.getHeight() - 2, 1, c.getHeight() - midRounding / 2
					- 2);
			gp2.lineTo(2 + midRounding / 2, c.getHeight() - 3);
			gp2.quadTo(2, c.getHeight() - 3, 2, c.getHeight() - midRounding / 2
					- 3);
		} else {
		...
		if (alwaysDrawBackground) {
			if (integer != null) {
				switch (integer) {
				case 0:
					Image min = new ImageIcon("src/Images/texture.jpg")
							.getImage();
					g2d.drawImage(min, 0, 0, null);
					break;
				case 3:
					g2d.setPaint(c.isEnabled() ? staticBorderColor
							: staticDisabledBorderColor);
					g2d.draw(gp0);
					break;
				default:
					g2d.setPaint(new GradientPaint(0, 0,
							c.isEnabled() ? staticTopBg : disabledStaticTopBg,
							0, c.getHeight(), c.isEnabled() ? staticBottomBg
									: disabledStaticBottomBg));
					g2d.fill(gp0);
					g2d.setPaint(c.isEnabled() ? staticBorderColor
							: staticDisabledBorderColor);
					g2d.draw(gp0);
		...
		if (c.isEnabled()) {
			...
			if (model.isSelected() || model.isArmed()) {
				g2d.setPaint(new GradientPaint(c.getWidth() / 3,
						c.getHeight() / 3, topBg, c.getWidth(), c.getHeight(),
						bg));
				g2d.fill(gp0);
			} else if (mouseOver || fadeTime != maxFadeTimes) {
				g2d.setComposite(AlphaComposite.getInstance(
						AlphaComposite.SRC_OVER, mouseoverTransparency
			...
			// Рисуем кайму
			if (model.isSelected() || model.isArmed() || mouseOver
					|| fadeTime != maxFadeTimes) {
				g2d.setPaint(model.isSelected() ? selectedBorderColor
						: borderColor);
				g2d.draw(gp0);
	...
	public static MyButtonUI setupButtonUI(AbstractButton button,
		...
		button.setBorder(BorderFactory.createEmptyBorder(insets.top,
	...
	public static MyButtonUI setupDialogButtonUI(AbstractButton button,
		...
		gbui.setStaticBorderColor(new Color(155, 155, 155), new Color(175, 175,
				175));
